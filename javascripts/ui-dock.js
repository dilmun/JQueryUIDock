...
$.widget('ui.dock', {
  ...
  options : {
    ...
    // Callbacks
    'tabActivateCallback' : false,
    'resizeCallback' : false,
    'mergeStartCallback' : false,
    'mergeEndCallback' : false,
    'closeViewCallback' : false,
 ...
 * and views added to the widget. It also takes care of 
 * operations involving multiple containers and docks.
 * 
 * @constructor
 */
function MasterContainer(rootElement, options) {
  ...
  var armed = false; // True if tracking is in 
  var moved = false; // 
  
  // Callbacks
  var closeViewCallback = options['closeViewCallback'];
  
  /** For a view to be correctly handled inside the widget,
   * it must first be added to the master. */
  ...
  /** Opening a view will activate the relevant tab in the
   * dock that's currently holding it, making it visible. */
  this.openView = function(viewID) {
    if (!(viewID in this.views))
      $.error('Invalid view identifier ' + viewID);
    
    var view = views[viewID];
    view.getDock().openView(view);
  };
  
  /** Activated by clicking on a close icon. */
  this.deleteView = function(viewID) {
    if (!(viewID in this.views))
      $.error('Invalid view identifier ' + viewID);
    
    var view = this.views[viewID];
    var dock = view.getDock();
    dock.closeView(viewID);
    dock.checkEmpty();
    
    delete this.views[viewID];
    
    if (closeViewCallback)
      closeViewCallback(view);
  };
  
  /** Registering docks is necessary for cross-container
   * interactions. */
  ...
  /** Fired when a tab is dropped on any tablist 
   * (sortable receive event). */
  ...
  /** Transfers a view from one dock to another. */
  ...
  /** Notifies docks to start watching mouse movements. */
  this.armTrackers = function(origin, event, ui) {
    armed = true;
    moved = false;
    for (var id in docks) {
      var dock = docks[id];
      ...
      if (dock.tracker)
        dock.tracker.startTracking(origin);
    }
  };
  
  /** Notifies docks that mouse dragging has stopped, triggering
   * various checks. */
  ...
  // The master container is also a split container
...
/**
 * A Dock object is a wrapper to a regular jQUery UI Tabs widget, 
 * providing much of the functionality in this plugin, namely :
 * - adding and closing tabbed views
 * - watching drag & drop events via an attached DropTracker
 * object (see below) and triggering dock split actions
 * - auto-destroy when all views have been removed
 * 
 * A Dock always has a SplitContainer parent which handles the 
 * actual splitting behaviour.
 * 
 * @constructor
 */
function Dock(master, parent) {
  ...
  /** The jQuery UI Sortable API is not only used 
   * to enable sortable tabs, but also to handle all drag & drop
  ...
  var sortable_cfg = {
    ...
    // Callbacks : forwarded to the master
    start: function(event, ui) {
      master.armTrackers(self, event, ui);
    },
  ...
  var tabActivateCallback = master.options['tabActivateCallback'];
  var tabs_cfg = {
    activate: function(event, ui) {
      active = ui.newTab.attr('data-view-id');
      if (tabActivateCallback)
        tabActivateCallback(event, ui);
    }
  };
  ...
  /** When a view is dropped in a splittable region, the 
   * current tab is destroyed and recreated in the newly formed dock */
  ...
  /** Used when the tab has already been dropped in a 
   * connected sortable tablist and thus doesn't need further
   * processing */
  ...
  /** If no view remains, destroy this dock and notify the parent 
   * SplitContainer */
  ...
  // Reapply jQuery UI widgets on the tablist
  ...
  // Check if this dock needs a DropTracker and create / destroy accordingly
...
/**
 * SplitContainers may either contain a unique Dock child 
 * (the "leaf"), or two resizable children SplitContainer 
 * elements ("prime" and "second"), in which case the "isSplit()" 
 * method returns true. Resizing (and related bonus features) is
 * delegated to the jQuery Layout plugin.
 */
function SplitContainer(master, parent, dock, $elem) {
  ...
  this.merge = function(closed) {
    ...
    var mergeStartCallback = master.options['mergeStartCallback'];
    if (mergeStartCallback)
      mergeStartCallback(closed.$elem, salvaged.$elem);
    ...
    var mergeEndCallback = master.options['mergeEndCallback'];
    if (mergeEndCallback)
      if (this.isSplit())
        mergeEndCallback(true, this.prime.$elem, this.second.$elem);
      else
        mergeEndCallback(false, this.$elem);
  };
  ...
  function applyLayout() {
    ...
    var resizeCallback = master.options['resizeCallback'];
    ...
    if (resizeCallback)
      layout_cfg.onresize = resizeCallback;
    self.layout = self.$elem.layout(layout_cfg);
  }
}
...
function DropTracker(dock) {
  ...
  var $hl = $('<div>').addClass('ui-dock-highlight').css('display', 'none');
  
  var isTracking = false;
  var currentRegion = false;
  var map = null;
  
  var fadingDuration = master.options['highlightFade'];
  ...
  this.startTracking = function(origin) {
    if (origin == dock && Object.keys(dock.views).length == 1)
      return;
  ...
  this.endTracking = function(viewID, originID) {
    ...
    if (currentRegion !== false) {
      ...
      var origin = master.docks[originID];
      var view = origin.closeView(viewID);
